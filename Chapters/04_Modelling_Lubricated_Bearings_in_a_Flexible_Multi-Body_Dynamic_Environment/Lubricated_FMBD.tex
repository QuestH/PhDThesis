...
\begin{figure}
	\centering  
	\includegraphics[width=130mm]{FlexiTribo Figure 3. Bearing Schematic.png}
...
\end{equation}
Figure \ref{Dry vs lubricated roller-race contact} in Chapter \ref{Investigating the Effect of Lubrication on the Tribodynamic Behaviour of High-Speed Roller Bearings} demonstrates this more clearly. The total contact deformation is the summation of $\left(h_c\right)$ and the material deformation, $\left(\delta_m\right)$ predicted from the dry-Hertzian contact assumption. Conventional dry analysis only accounts for the dry material deformation at the contact.
...
The bearing geometry is detailed in Table \ref{Cylindrical Roller Bearing Specification}. Rheological and material properties are detailed in Table \ref{Bearing Rheological Properties}, representing a common transmission oil at ambient operating conditions in an individual hub-motor transmission.
...
The system level model is decomposed, with excitation forces calculated externally before application within the model. A separate electrified transmission, modelled in AVL EXCITE\textsuperscript{TM} M, is used to generate realistic excitation forces and torques from a spur gear pair and a permanent magnet synchronous motor (PMSM). This system represents the first stage of an electric hub motor used in automotive applications.
...
The motor delivers a maximum average torque of 68~$\mathrm{Nm}$ in the constant torque region, and a maximum operating speed of 21~000~$\mathrm{rpm}$. The transmitted torque reduces with increasing speed due to the torque–speed characteristic of the PMSM, as shown in Figure \ref{PMSM torque profile}. Radial stator tooth forces from the PMSM are neglected, as their resolved components in the lateral directions are negligible relative to the gear mesh excitation forces. For input to the model, the radial and tangential forces are simplified as sinusoidal excitations with the same magnitude and frequency as those generated by the gear pair at each operating speed. The torque ripple from the motor is simplified using the same method.
...
\begin{sidewaysfigure}
	\centering  
	\includegraphics[width=250mm]{Simulink_schematic_annotated.png}
...
These results demonstrate that conventional dry-contact bearing models underestimate stiffness and fail to capture speed-dependent behaviour arising from EHL film formation. By implicitly modelling the lubricant film within the rolling element contact, the proposed method captures the increase in film thickness with entrainment velocity and the resulting change in contact deflection and Hertzian contact force. This produces a higher effective contact stiffness at the roller–race interface. When the contribution of all rolling elements is considered, the increased contact stiffness propagates to the overall bearing stiffness. In the FMBD model, the bearing stiffness forms part of the global stiffness matrix of the rotor–bearing–housing system. Consequently, the increase in bearing stiffness alters the dynamic characteristics of the drivetrain, increasing the natural frequency of the flexible shaft and modifying the predicted NVH response. The results therefore show that tribological behaviour at the contact level can propagate to system-level dynamic behaviour, highlighting the importance of including EHL effects in high-speed electrified powertrain simulation

The central film thickness is the governing contributor to these stiffness disparities. It was, however, noted in  Chapter \ref{Investigating the Effect of Lubrication on the Tribodynamic Behaviour of High-Speed Roller Bearings}, Section \ref{Numerical vs analytical experimental} that the regressed equation overestimates the central film thickness when compared to the numerical solution by up to 11.3\% at 8~350~rpm. To further investigate this behaviour, explicit numerical analysis was performed at each speed point in the bearing presented within this chapter. The results of this are shown in Chapter \ref{ANN Lubricated Bearing FMBD}, Figure \ref{EHL_NumericalvsAnalytical_25000}. The analysis revealed that the calculated film values diverge with increasing rotational speed and hence entrainment velocity.
