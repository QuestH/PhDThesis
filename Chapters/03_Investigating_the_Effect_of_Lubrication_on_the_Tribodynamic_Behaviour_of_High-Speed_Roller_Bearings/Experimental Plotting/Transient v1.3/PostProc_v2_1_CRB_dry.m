...
%% Instantaneous deflection of each element
        ...
        Ht_in(i) = 0; %abs(delta_in(i)); % film still plots correctly, but load not calculated hydrodynamically if between 0 and 4 micron as this is not valid
        Ht_out(i) = 0; %abs(delta_out(i));
...
for i = lower+1:upper-1
            linear_stiff2(i) = (Wt_out(i+1)-Wt_out(i-1))/(delta_out(i+1)-delta_out(i-1));
        end
...
save Wt_out_dry.mat Wt_out speed
