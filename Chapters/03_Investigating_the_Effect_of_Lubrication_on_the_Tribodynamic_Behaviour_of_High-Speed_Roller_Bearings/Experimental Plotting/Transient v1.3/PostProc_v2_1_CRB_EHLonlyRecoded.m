...
%% Instantaneous deflection of each element
        ...
        % Initial guesses
        W_tin(i) = 50; % initial guess! instantaneouse reaction load of each element
        W_tout(i) = 50; % initial guess! instantaneouse reaction load of each element
        film(i) = 1e-6;
        filmNEW(i) = 1e-6;
        filmNEW_in(i) = 1e-6;
        filmNEW_out(i) = 1e-6;
        ...
        Ge = Er*alpha;
        We_in = W_tin(i)/(Er*Rrin*length);
        We_out = W_tout(i)/(Er*Rrout*length);
