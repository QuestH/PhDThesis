...
This chapter presents a high-speed experimental test rig which has been instrumented to experimentally measure the kinematic motion of a bearing at rotational speeds up to 15~000~$rpm$ (equating to an $nD_m$ of 5.78$~\times~10^{5}$~$mm/min$) and a radial load of 750~$N$. This motion was used as the boundary condition for tribological modelling. Roller-race contact load and film thickness are obtained using a lubricated contact model, whereby the EHL film is considered implicitly at the roller-race conjunction. EHL pressure and film thickness distributions are then obtained using an explicit 1D numerical EHL model. Fundamental equations used in the subsequent chapters are also introduced.

\subsection{Chapter Contributions}

The influence of EHL film formation on contact load, deflection, and stiffness in rolling element bearings is not widely investigated in literature for the high-speed, low-load operating conditions characteristic of modern electrified powertrains. Before assessing the system-level implications of this behaviour, the extent of the film contribution to contact mechanics must first be quantified.

This chapter addresses these limitations through the following contributions:
\begin{enumerate}
	\item Design and manufacture of a high-speed bearing test capable of resolving kinematic bearing motion under operating conditions representative of electrified powertrains. This negates the requirement for system level dynamic modelling in the investigative stages, whilst providing realistic bearing orbital motion for the loading conditions under investigation.
	\item Development of a coupled experimental-tribological modelling framework in which the bearing kinematics from the test rig are used as boundary conditions for a lubricated bearing model. Such direct coupling of high‑speed kinematic measurements with EHL-based contact analysis has not previously been reported at these operating conditions.
	\item Quantify the influence of the implicitly modelled EHL film on roller-race contact load, deflection, and stiffness, and demonstrate the limitations of conventional dry analyses in the high-speed regime.
\end{enumerate}
...
\begin{figure}
	\centering
    \includegraphics[width=150 mm]{ExpTribo Figure 1 - Methodology Overview.png}
...
\begin{figure}
	\centering
	\includegraphics[width=90 mm]{ExpTribo Figure 2 - CRB in equilibrium position.png}
...
The displacement of the bearing centre, which governs the conditions at the contact, was determined experimentally using a high-speed bearing test rig\footnote[1]{The author was responsible for the design, technical drawings, partial manufacture, assembly and operation of the test rig.}. Figure \ref{Experimental rig schematic} shows a schematic of the rig.

\paragraph{Motor}
A 5~$kW$ AC synchronous motor, capable of speeds up to 32~000~rpm was coupled to a steel shaft supported by two bearing brackets. A CNC spindle was selected as the drive motor due to its low axial and radial runout, minimising harmonics originating from the drive unit itself.

\paragraph{Shaft Coupling}
The motor was connected to the shaft using a polymeric jaw coupling (Ondrives OKBE3/19), rated for speeds up to 19~000~$rpm$ to provide sufficient margin above the maximum required test speed. The 98~$ShA$ polyurethane insert provided torsional flexibility whilst maintaining adequate stiffness for high‑speed operation. It was selected to accommodate small angular and parallel misalignment and to limit the transmission of torsional vibration to the shaft.

\paragraph{Alignment}
For the operating speeds considered, the permissible coupling alignment limits were specified as 0.01~$mm$ per 100~$mm$ for angular misalignment and 0.01~$mm$ for parallel offset. Although tight dimensional tolerances were applied during the design and manufacture, these alone could not guarantee coaxial alignment at this precision. Laser alignment was therefore performed to correct both forms of misalignment, ensuring the shafts operated within permissible limits and to prevent excitation of higher‑order vibration modes at elevated speeds.

\paragraph{Radial Force Application}
Radial force was transferred to the shaft via a hinge‑arm mechanism, as is common in similar applications \cite{Cockburn1995a}. The force was transferred via a point‑load applicator. A point contact was selected to avoid the use of an auxiliary support bearing at the load application location, such as in \cite{Flack1993a}. This would introduce additional excitation frequencies within the measurement bandwidth. The applied radial force was calibrated using a permanently preloaded force transducer (B\&K Type 8200), capable of measuring up to 5000~$N$ in compression.

\paragraph{Shaft}
The shaft was manufactured from SAE~8620 steel due to its favourable combination of tensile and yield strength, as well as surface wear resistance. The load application area was finish‑turned and polished to a surface roughness of 0.1~–~0.2~$\mu m$ to minimise the risk of fretting or wear under repeated point loading. A diamond‑like carbon (DLC) coating was additionally applied to further enhance wear resistance and reduce friction.

\paragraph{Bearing Bracket}
Bearing centre displacement was measured using an instrumented bearing bracket. The bracket, machined from 7075 aluminium, was based on an AVL e‑Axle endplate design. Modal analysis was performed to ensure that the first mode of vibration would not be excited by the rotary frequency of the shaft. This mode was calculated to be 1120.5~$Hz$; well above both the shaft’s first‑order excitation at 15~000~$rpm$ (300~$Hz$) and its first natural frequency (211~$Hz$), corresponding to a critical speed of 12,660~$rpm$.

\paragraph{Bearing}
The bearing under test was a single-row cylindrical roller bearing, NU~205~ECP. The same bearing was used in the support bracket. This had a high-speed $nD_m$ value of 5.78$~\times~10^{5}$~$mm/min$ at its operating speed of 15~000~$rpm$.

\begin{figure}
	\centering
	\includegraphics[width=150 mm]{ExpTribo Figure 5 – Experimental rig schematic.png}
...
The motor control unit was connected to a voltage input, transmitted via an NI cDAQ-9178 USB chassis for optimum resolution. A MATLAB\textregistered\ script controlled the voltage ramp over a specified time-period and thus the spindle acceleration. In this study, a transient speed sweep was performed from 0~–~15~000~$rpm$ over a 4~$s$ period, with a voltage resolution of 1~$V$~/~3~500~$rpm$.

\subsection{Instrumentation}
The outer surface of the bearing bore was instrumented with two Type~4383 single-axis piezo-electric charge accelerometers, with a frequency range of 0.1~–~8.50$~kHz$ and sensitivity of 3.16~$pC/ms^{-2}$.  These measured acceleration of the bracket’s bore, corresponding to the outer race of the bearing (Figure \ref{Accelerometer locations on test bracket}). The Type~4383 accelerometer resolution is governed by the residual noise floor of 0.06~$mg$ (rms) over the useable frequency range. When converted via double integration (see Section \ref{Signal processing}), this noise corresponds to a displacement resolution of 150~$nm$ at 300~$Hz$ (the shaft order at 15~000~$rpm$).

Two single beam laser vibrometers measured the displacement of the shaft at the edge of the bearing which corresponds to the displacement of the inner race of the bearing (Figure \ref{Laser vibrometer locations on shaft}). Laser vibrometers offered the advantage of non-interfering (stiffness/mass) measurement and allowed for radial displacement to be obtained from a rotating shaft. The Polytec OFV-505 vibrometers combined with an OFV-5000 controller have a displacement resolution of 0.15~$nm$. A Polytec RLV-5500 dual-beam vibrometer with a frequency range of 0~–~10~$kHz$ was used to measure the rotational speed of the shaft.

\begin{figure}
	\centering
    \includegraphics[width=135 mm]{ExpTribo Figure 6(a) - Accelerometer Locations on Test Bracket.png}
...
\begin{figure}
	\centering
	\includegraphics[width=135 mm]{ExpTribo Figure 6(b) - Laser Vibrometer Locations on Shaft.png}
	\caption{Laser vibrometer locations on shaft.}
	\label{Laser vibrometer locations on shaft}
\end{figure}

Data was simultaneously acquired from the accelerometers and laser vibrometers through synchronised input channels at a sampling rate of 100~$kHz$. Simultaneous control and data acquisition ensured accurate results between different instrumentation locations.
...
Double integration is inherently sensitive to low‑frequency drift, DC (direct current) offsets, and non-linearities in the original acceleration signal. This can lead to displacement errors if not pre-processed \cite{Takeda2014}. To suppress this drift, a linear trend was removed from the raw acceleration data before integration. This detrending specifically targetted frequencies below 50~-~70~$Hz$, where the Type~4383 accelerometer exhibits the highest noise amplification under double integration. This frequency range was below the measurement band of interest.

To extract the frequency content of the speed ramp, waterfall plots were generated using the accelerometer and laser vibrometer data. Short-time fast-Fourier transform (STFFT) was used, with data windows of six shaft rotations at each 100~$rpm$ increment. Using a fixed number of rotations per window ensured consistent shaft‑order resolution across the full speed range, whilst six rotations provided sufficient cycles for adequate spectral resolution without degrading time localisation. The 100~$rpm$ increment was small enough to track gradual changes in contact dynamics during acceleration whilst retaining computational efficiency of result processing.

The resulting signal was then filtered using a Butterworth bandpass filter for a frequency range of 70~-~10~000~$Hz$ to to retain the bearing‑related dynamic content whilst suppressing low‑frequency equipment noise and high‑frequency artefacts. The lower cutoff at 70~$Hz$ eliminates the frequency region where double‑integration drift dominates, whilst the upper cutoff of 10~$kHz$ lies safely within the accelerometer’s linear operating bandwidth, which is specified as 0.1~–~8~400~$Hz$. The Butterworth response was selected for its maximally flat passband, ensuring no artificial gain or attenuation of the displacement amplitudes. The stopbands were set at 35~$Hz$ and 12~500~$Hz$, each with 7~$dB$ attenuation and a 3~$dB$ passband ripple, providing smooth roll‑off characteristics and preserving physical displacement information throughout the full operating range.
...
\begin{enumerate}
	...
	\end{equation}
	An iterative process is used to calculate load on the roller based on total deflection including lubricant film (Equation \ref{Contact deflection experimental}). At each time step where an EHL film is present, the following convergence criteria must be met before the next time step is calculated. Because the influence of contact load on film thickness is small in comparison to entraining motion (see Equation \ref{dimensionless central film thickness}), the solution is only weakly sensitive to load updates. Therefore, the convergence tolerance can be relaxed to reduce computational cost:
	\begin{equation}\label{film convergence}
		\frac{h_i^n-h_i^{n-1}}{h_i^{n-1}} \leq 0.01
	\end{equation}
\end{enumerate}

As demonstrated in Section \ref{{Thermal EHL}}, inlet shear heating can cause the central film thickness to deviate from the isothermal predictions at high entrainment velocities. In the present study, the maximum entrainment velocity is 7.5~$m \cdot \mathrm{s}^{-1}$. At this speed, assuming pure-rolling, the film reduction predicted by Gupta \textit{et al.} \cite{Gupta1992} remains small relative to the overall magnitude of the film, and more importantly, is negligible compared with the difference in contact deformation between dry and lubricated analyses. Consequently, the incremental reduction in film thickness induced by inlet shear heating is not expected to materially affect the comparative contact force results that underpin this investigation. The lubricant film in both the analytical and numerical investigations in this study are modelled as isothermal.
...
The analysis of the film behaviour in this thesis is performed under isothermal, steady-state conditions. The methodology for the 1D EHL solution is as follows.
...
\begin{equation}\label{eq3.33}
	\frac{\partial}{\partial X}\left[\frac{\bar{\rho} H^3}{6 \bar{\eta}}\left(\frac{\partial P}{\partial X}\right)\right]=\Psi\left[\frac{\partial}{\partial X} \bar{\rho} H U\right]
\end{equation}
where $X$ is the direction of entraining motion into the contact. 

In the absence of shock loading, squeeze film motion is neglected for this analysis. The pressure-density relationship in the compressible model is modelled using Dowson-Higginson \cite{Dowson1977} (Equation \ref{eq3.29}). Roelands equation \cite{Roelands1966} (\ref{eq3.25}) is used for the pressure-viscosity relationship.
...
\begin{figure}
	\centering
	\includegraphics[width=150mm]{ExpTribo Figure 14 – EHL and hydrodynamic conditions during roller operation_2.png}
...
\begin{figure}
	\centering
	\includegraphics[width=150mm]{ExpTribo Figure 15 - Contact Load Difference Between Dry and Lubricated Model_2.png}
...
A novel methodology comprising experiments and numerical modelling was developed to allow component and conjunction level tribodynamic analysis of a roller bearing under speeds and loading conditions previously not reported. The experimental data contain the physics of the dynamics and tribology within the bearing, negating the need for a simplified and computationally intensive dynamic bearing model. The tribological conditions at the contact between an individual roller and raceways are numerically analysed. EHL and hydrodynamic lubrication regimes are considered as the roller passes through loaded and unloaded regions. The following conclusions can be made based on presented results:

\begin{enumerate}
	\item The contact experiences an order of magnitude increase in film thickness (0.1~-~1.9~$\mu m$) across the speed sweep from 0~-~15~000$rpm$. The film contributes directly to the interference at roll-race contact and increases with speed. For a given bearing displacement, the predicted contact reaction force increases because the load is now generated by both the Hertzian elastic deformation and the additional separation imposed by the elastohydrodynamic film.	This leads to a 1~200~$N$ higher contact load for the analysis considering the EHL film implicitly when compared to the conventional dry modelling approach.
...
The results demonstrate that neglecting the elastohydrodynamic film in bearing contact models can lead to a significant under prediction of contact load. At high speeds and low external loads, the effect of the EHL film is shown to considerably affect the resulting contact forces. These operating conditions align with those of an input bearing in an electrified powertrain transmission, where input torque and hence bearing load decreases as rotational speeds increase. Moreover, although the growth of the lubricant film improves surface separation and reduces the likelihood of asperity contact, its contribution to the effective contact interference increases the internal load within the rolling element–raceway conjunction. Consequently, dry contact models may produce non-conservative life estimates under high-speed, low-load operating conditions.
