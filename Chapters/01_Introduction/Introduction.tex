...
To achieve high-efficiency and adhere to packaging constraints, modern electrified powertrains utilize high-speed and low load motors \cite{Cai2021}. These motors introduce new challenges regarding NVH (Noise, Vibration and Harshness) and the tribology of interacting conjunctions. The compact, lightweight and efficient powertrains operate under significantly different working conditions to conventional internal combustion engine (ICE) systems. They are subject to different underlying physics; such as regime of lubrication, dynamic response and magneto-mechanical interactions.

This style of powertrain architecture involves high-speed rolling element bearing operation. The motor and transmission input shaft bearings typically operate at rotational speeds up to 20~000~$rpm$, with advanced designs achieving 30~000~$rpm$. For typical mean diameters of 35~-~50~$mm$, this corresponds to $nDm$ speed factor values of 0.5~-~1.2$~\times~10^6$~$mm/min$, equating to entrainment velocities approaching 30~$m/s$.
...
\section{Research Aim and Objectives} \label{Research Questions}

The primary aim of this thesis is to evaluate whether implicit inclusion of the elastohydrodynamic lubrication (EHL) film at the roller–race contact is required for accurate dynamic modelling of high-speed rolling element bearings in electrified powertrains. It also aims to establish a computationally efficient and accurate method for its implementation.

To achieve this aim, the following research objectives and tasks were defined: 
...
\begin{enumerate}
	...
	\item To determine the extent to which implicit EHL film inclusion modifies system-level dynamic behaviour within a flexible multi-body dynamic (FMBD) environment.
	\item To evaluate the accuracy and computational suitability of artificial neural networks (ANNs) for predicting central EHL film thickness across a representative operating domain.
	\item To determine whether ANN surrogate models can provide a viable embedded alternative to conventional film thickness formulations within FMBD environments.
\end{enumerate}

\paragraph{Research Tasks}
\begin{enumerate}
	\item Design and manufacture a high-speed bearing test rig capable of operating up to the defined speed and load conditions of electrified powertrains. Instrument the rig to measure bearing orbital motion and establish a repeatable data acquisition and signal processing methodology. 
	\item Extract time-domain kinematic boundary conditions from experimental measurements for use in tribological conjunction modelling.
	\item Develop a lubricated component-level bearing model, including implicit representation of the EHL film at the roller–race contact. Compare contact force and stiffness results with the conventional dry bearing modelling approach.
	\item Develop a coupled co-simulation framework to integrate the lubricated bearing model within an FMBD environment. Compare bearing stiffness and system-level dynamic response with the conventional dry bearing modelling approach.
	\item Implement a numerical EHL model to compute central film thickness across the required operating domain.
	\item Generate a structured training dataset using regime-constrained sampling of the data from the numerical EHL model.
	...
	\item Embed the trained ANN within the FMBD model and benchmark computational efficiency and predictive accuracy against the numerical and anlytical solutions.
\end{enumerate}
...
This work was performed in collaboration with AVL List GmbH in order to disseminate the outcomes of this research in the development of commercial software: AVL EXCITE\textsuperscript{TM} M and AVL EXCITE\textsuperscript{TM} Power Unit.
